...
use TestApp\Event\EntrySavedEvent;
...
abstract class BrowserImport extends AbstractImport
{
    ...
    {
        if (!$this->user) {
            $this->logger->error('TestApp Browser Import: user is not defined');

            return false;
        }

        if (!file_exists($this->filepath) || !is_readable($this->filepath)) {
            $this->logger->error('TestApp Browser Import: unable to read file', ['filepath' => $this->filepath]);
        ...
        if (empty($data)) {
            $this->logger->error('TestApp Browser: no entries in imported file');
